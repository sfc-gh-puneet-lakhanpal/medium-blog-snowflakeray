...
from snowflakeray.deploy_client.image_registry import credential
from snowflakeray.deploy_client.image_builds import base_image_builder
...
class ClientImageBuilder(base_image_builder.ImageBuilder):
    ...
    def build_and_upload_image(self) -> None:
        ...
        with credential.generate_image_registry_credential(
            ...
            try:
                ...
                raise Exception(f"Failed to build docker image. Error {str(e)}") from e
            else:
                try:
                    start = time.time()
                    ...
                    raise Exception("Failed to upload docker image to registry.") from e
                finally:
    ...
    def validate_docker_client_env(self) -> None:
        ...
        try:
            ...
            raise Exception("Failed to initialize Docker client. Please ensure Docker is installed and running.") from e

        try:
            self._run_docker_commands(["docker", "buildx", "version"])
        except Exception as e:
            raise Exception("Please ensured Docker is installed with BuildKit by following https://docs.docker.com/build/buildkit/#getting-started") from e
    ...
    def _run_docker_commands(self, commands: List[str]) -> None:
        ...
        if proc.wait():
            for line in output_lines:
                logger.error(line)
            raise Exception(f"Docker command failed: {' '.join(commands)}")
